...
**URL:** `https://discount-system-fix.preview.emergentagent.com/api/appointments/couple`
...
async function submitCoupleBooking(event) {
  ...
  try {
    const response = await fetch(
      'https://discount-system-fix.preview.emergentagent.com/api/appointments/couple',
      {
        method: 'POST',
...
```bash
curl -X POST https://discount-system-fix.preview.emergentagent.com/api/appointments/couple \
...
6. Kada korisnik submituje, pošalji na:
   POST https://discount-system-fix.preview.emergentagent.com/api/appointments/couple
