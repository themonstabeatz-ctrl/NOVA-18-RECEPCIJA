...
**URL:** `https://price-consistency.preview.emergentagent.com/api/appointments/couple`
...
async function submitCoupleBooking(event) {
  ...
  try {
    const response = await fetch(
      'https://price-consistency.preview.emergentagent.com/api/appointments/couple',
      {
        method: 'POST',
...
```bash
curl -X POST https://price-consistency.preview.emergentagent.com/api/appointments/couple \
...
6. Kada korisnik submituje, pošalji na:
   POST https://price-consistency.preview.emergentagent.com/api/appointments/couple
