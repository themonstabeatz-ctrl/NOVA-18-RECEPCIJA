...
Povezati websajt `https://buluang-spa-fix.preview.emergentagent.com/` sa booking sistemom `https://spabooking.preview.emergentagent.com/` da:
...
function MasazePage() {
  ...
  useEffect(() => {
    // Učitaj sve usluge
    fetch('https://spabooking.preview.emergentagent.com/api/services')
      .then(res => res.json())
      .then(data => {
