...
```javascript
// Fetch SVE servise
const services = await fetch('https://backend-pricing.preview.emergentagent.com/api/services')
  .then(r => r.json());
...
const allServices = await fetch('https://backend-pricing.preview.emergentagent.com/api/services')
  .then(r => r.json());
